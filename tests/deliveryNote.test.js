...
describe('DeliveryNote API Tests', () => {
  ...
  describe('GET /api/deliverynote', () => {
    ...
    it('should get all delivery notes successfully with exact structure', async () => {
      ...
      expect(new Date(res.body.data[0].date)).toBeGreaterThan(new Date(res.body.data[1].date));
  ...
  describe('POST /api/deliverynote/:id/sign', () => {
    ...
    it('should sign delivery note successfully', async () => {
      ...
      expect(res.body.message).toBe('Delivery note signed successfully.');
      expect(res.body.data.isSigned).toBe(true);
      expect(res.body.data.status).toBe('signed');
      expect(res.body.data.signedDate).toBeDefined();
      expect(res.body.data.signerName).toBe('John Client');
      expect(res.body.data.signerTitle).toBe('Project Manager');
      expect(res.body.data.signatureUrl).toContain('ipfs://');
      expect(res.body.data.pdfUrl).toContain('ipfs://');
      expect(res.body.data.signatureGatewayUrl).toContain('https://');
      expect(res.body.data.pdfGatewayUrl).toContain('https://');
    });
    ...
    it('should fail with invalid delivery note ID', async () => {
      ...
      const res = await request(app)
        .post(`/api/deliverynote/${nonExistentId}/sign`)
        .set('Authorization', `Bearer ${userToken}`)
        .send(signData)
        .expect(404);
